...
class UNetModel(nn.Module):
    ...
    def forward(self, input):
        ...
        if ("grounding_input" in input):
            ...
            grounding_input = self.batch_to_device(grounding_input, self.device)
        else: 
            # Guidance null case
            grounding_input = self.grounding_tokenizer_input.get_null_input(device=self.device)
        ...
        # Time embedding 
        t_emb = timestep_embedding(input["timesteps"].to(self.device), self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)

        # input tensor  
        h = input["x"].to(self.device)
        if self.downsample_net != None and self.first_conv_type=="GLIGEN":
            temp  = self.downsample_net(input["grounding_extra_input"].to(self.device))
            h = th.cat( [h,temp], dim=1 )
        if self.inpaint_mode and input["inpainting_extra_input"] is not None:
            #if self.downsample_net != None:
            #    breakpoint() # TODO: think about this case 
            h = th.cat( [h, input["inpainting_extra_input"].to(self.device)], dim=1 )
        ...
        for module in self.input_blocks:
            h = module(h, emb, context.to(self.device), objs)
            hs.append(h)

        h = self.middle_block(h, emb, context.to(self.device), objs)
