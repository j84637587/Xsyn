...
class Trainer:
    def __init__(self, config, device):
        ...
        self.model.position_net = self.model.position_net.to(self.device)
    ...
    def get_input(self, batch):
        ...
        if self.config.inpaint_mode:
            ...
            inpainting_mask = draw_masks_from_boxes(batch['boxes'], 64, randomize_fg_mask=self.config.randomize_fg_mask, random_add_bg_mask=self.config.random_add_bg_mask, arbitrary_mask_percent=0.5).to(self.device)
        ...
        '''
        if self.config.inpaint_mode:
            masked_z = self.autoencoder.encode( batch["inpaint_image"] )
            inpainting_extra_input = torch.cat([masked_z, Resize([z.shape[-1],z.shape[-1]])(batch['inpaint_mask'])], dim=1) 
        '''
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            if self.config.inpaint_mode:
                z = self.autoencoder.encode( batch["image"] )
                inpainting_mask = draw_masks_from_boxes(batch['boxes'], 64, randomize_fg_mask=self.config.randomize_fg_mask, random_add_bg_mask=self.config.random_add_bg_mask).to(self.device)
                masked_z = z*inpainting_mask
                inpainting_extra_input = torch.cat([masked_z,inpainting_mask], dim=1)
